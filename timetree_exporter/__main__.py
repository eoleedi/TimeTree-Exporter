...
def main():
    ...
    with Path(args.output).open("wb") as f:  # Path Traversal Vulnerability if on a server
        f.write(cal.to_ical())
        logger.info("The .ics calendar file is saved to %s", Path(args.output).resolve())
